...
export const getUserApiScript = async (id: string): Promise<string> => {
  ...
  return normalizeUserApiScript(script)
}
...
const normalizeUserApiScript = (script: string): string => {
  if (typeof script != 'string') return script
  const trimScript = script.trim()
  if (!trimScript.startsWith('gz_')) return script

  try {
    const compressed = Buffer.from(trimScript.substring(3), 'base64')
    return ungzip(compressed, { to: 'string' })
  } catch (error: any) {
    throw new Error(`user api script decompress failed: ${error.message}`)
  }
}

const normalizeUserApiScripts = (scripts: Record<string, string>): Record<string, string> => {
  const normalized: Record<string, string> = {}
  for (const [id, script] of Object.entries(scripts ?? {})) {
    normalized[id] = normalizeUserApiScript(script)
  }
  return normalized
}

export { normalizeUserApiScripts }
...
export const addUserApi = async (script: string): Promise<LX.UserApi.UserApiInfo> => {
  const normalizedScript = normalizeUserApiScript(script)
  const result = /^\/\*[\S|\s]+?\*\//.exec(normalizedScript)
  ...
  await saveDataMultiple([
    ...
    [`${userApiPrefix}${apiInfo.id}`, normalizedScript],
  ])
  return apiInfo
}
