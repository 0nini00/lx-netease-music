...
const mergeSetting = (
  ...
  if (targetSetting) {
    const targetRecord = targetSetting as Record<string, any>
    // 兼容旧配置：旧版只保存 common.music_api_gateway_enable，初始化时迁移到新的播放方式字段。
    if (
      targetRecord['common.music_play_use_gateway'] == null &&
      targetRecord['common.music_api_gateway_enable'] != null
    ) {
      targetRecord['common.music_play_use_gateway'] = !!targetRecord['common.music_api_gateway_enable']
    }
