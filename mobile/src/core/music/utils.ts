...
import { getMusicApiGatewayUrl, isMusicApiGatewayEnabled } from '@/core/musicApiGateway'
...
const buildSourceFallbackQualitys = ({
  ...
  appendQuality(getPlayQuality(settingState.setting['player.playQuality'], musicInfo))
...
export const getOnlineOtherSourceMusicUrl = async ({
  ...
  while ((musicInfo = musicInfos.shift()!)) {
    ...
    itemQualitys = buildSourceFallbackQualitys({
      musicInfo,
      quality: quality ?? getPlayQuality(settingState.setting['player.playQuality'], musicInfo),
      sourceQualitys,
    })
  ...
  // 轮询兜底会按当前请求音质、用户偏好音质、音源支持音质依次尝试，避免某个备用音源缺少单一音质就被整体跳过。
