...
const normalizeUserApisPayload = (data: { list: LX.UserApi.UserApiInfo[], scripts: Record<string, string> }) => ({
  ...data,
  scripts: normalizeUserApiScripts(data.scripts),
})
...
export const setUserApi = async(apiId: string) => {
  ...
  if (typeof script != 'string' || !script.trim()) {
    throw new Error('user api script is empty')
  }
...
export const overwriteUserApis = async(data: { list: LX.UserApi.UserApiInfo[], scripts: Record<string, string> }) => {
  try {
    ...
    saveTasks.push([storageDataPrefix.userApi, normalizedData.list])
    for (const [id, script] of Object.entries(normalizedData.scripts)) {
      saveTasks.push([`${storageDataPrefix.userApi}${id}`, script])
    }
    await saveDataMultiple(saveTasks)

    action.setUserApiList(normalizedData.list)
  } catch (error: any) {
