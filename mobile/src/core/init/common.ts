...
import { setWyLikedSongs, setWySubscribedPlaylists, setWyUid } from '@/store/user/action'
...
export default async(setting: LX.AppSetting) => {
  ...
  const refreshWyCookieData = async(cookie: string) => {
    const targetCookie = cookie.trim()
    if (!targetCookie) {
      setWyUid(null)
      setWyLikedSongs([])
    ...
    const accountStatus = await wyUserApi.getAccountStatus(targetCookie)
    const uid = String(accountStatus.uid)
    setWyUid(uid)

    const [likedIds, playlists] = await Promise.all([
      wyUserApi.getLikedSongList(uid, targetCookie).catch(() => []),
      wyUserApi.getUserPlaylists(uid, targetCookie).catch(() => []),
    ])

    setWyLikedSongs(likedIds as any)
    setWySubscribedPlaylists(playlists as any)
  ...
  const handleWyCookieUpdate = (keys: Array<keyof LX.AppSetting>, setting: Partial<LX.AppSetting>) => {
    ...
    void refreshWyCookieData(setting['common.wy_cookie'] ?? '').catch((err: any) => {
      setWyUid(null)
      setWyLikedSongs([])
      setWySubscribedPlaylists([])
