...
import { getWyAccountCachedPlaylistMusics, getWyDailyRecommendCachedMusics, isWyAccountListId, isWyDailyRecommendListId } from '@renderer/store/wyAccountPlaylist'
...
export const getList = (listId: string | null): Array<LX.Music.MusicInfo | LX.Download.ListItem> => {
  if (isWyDailyRecommendListId(listId)) return getWyDailyRecommendCachedMusics()
  if (isWyAccountListId(listId)) return getWyAccountCachedPlaylistMusics(listId)
