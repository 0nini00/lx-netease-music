...
import { addWyAccountPlaylistMusics, canAddToWyAccountPlaylist, getWritableWyAccountPlaylistTargets, isWyAccountListId, loadWyAccountPlaylists } from '@renderer/store/wyAccountPlaylist'
...
export default {
  ...
  setup(props) {
    ...
    const targetMusicList = computed(() => {
      return props.musicList.length && 'progress' in props.musicList[0] ? props.musicList.map(t => t.metadata.musicInfo) : props.musicList
    })
    const lists = computed(() => {
      ...
      const wyLists = props.isMove || !canAddToWyAccountPlaylist(targetMusicList.value)
        ? []
    ...
    watch(() => props.show, show => {
      if (!show || !canAddToWyAccountPlaylist(targetMusicList.value)) return
      void loadWyAccountPlaylists().catch(err => {
        console.log(err)
      })
    })
  ...
  methods: {
    ...
    handleClick(index) {
      ...
      const list = this.targetMusicList

      if (isWyAccountListId(targetList.id)) {
        void addWyAccountPlaylistMusics(targetList.id, list).catch(async err => {
          console.log(err)
          await dialog(err instanceof Error ? err.message : String(err))
        })
      } else if (this.isMove) void moveListMusics(this.fromListId, targetList.id, list)
      else void addListMusics(targetList.id, list)
