...
import { addWyAccountPlaylistMusics, canAddToWyAccountPlaylist, getWritableWyAccountPlaylistTargets, isWyAccountListId, loadWyAccountPlaylists, wyAccountPlaylistState } from '@renderer/store/wyAccountPlaylist'
...
export default {
  ...
  setup(props) {
    ...
    const getList = () => {
      ...
      const wyLists = props.isMove || !canAddToWyAccountPlaylist([musicInfo])
        ? []
        : getWritableWyAccountPlaylistTargets()
      lists.value = [
        ...localLists,
        ...wyLists,
    ...
    watch(() => props.show, show => {
      ...
      getList()
      if (canAddToWyAccountPlaylist([currentMusicInfo.value])) {
        void loadWyAccountPlaylists().then(getList).catch(err => {
          console.log(err)
        })
      }
      ...
      const stopWatchUserList = watch(userLists, getList)
      const stopWatchWyList = watch(wyAccountPlaylistState.list, getList)
    })
  ...
  methods: {
    ...
    handleClick(index) {
      const targetList = this.lists[index]
      if (isWyAccountListId(targetList.id)) {
        void addWyAccountPlaylistMusics(targetList.id, [this.currentMusicInfo]).catch(async err => {
          console.log(err)
          await dialog(err instanceof Error ? err.message : String(err))
          targetList.isExist = false
        })
      } else if (this.isMove) void moveListMusics(this.fromListId, targetList.id, [this.currentMusicInfo])
      else void addListMusics(targetList.id, [this.currentMusicInfo])

      targetList.isExist = true
