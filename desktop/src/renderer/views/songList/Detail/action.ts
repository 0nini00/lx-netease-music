...
import { getWyAccountPlaylistInfoByPlaylistId, loadWyAccountPlaylists, subscribeWyPlaylist } from '@renderer/store/wyAccountPlaylist'
...
export const addSongListDetail = async(id: string, source: LX.OnlineSource, name?: string) => {
  ...
  if (source == 'wy') {
    await loadWyAccountPlaylists()
    const playlistInfo = getWyAccountPlaylistInfoByPlaylistId(id)
    if (playlistInfo) {
      await dialog(window.i18n.t(playlistInfo.subscribed ? 'wy_account_playlist__already_subscribed' : 'wy_account_playlist__own_playlist'))
      return
    }

    await subscribeWyPlaylist(id, true)
    await dialog(window.i18n.t('wy_account_playlist__subscribe_success'))
    return
  }
