...
import { getWyAccountPlaylistInfo, isWyAccountListId, isWyDailyRecommendListId } from '@renderer/store/wyAccountPlaylist'
...
export default ({
  ...
  const showMenu = (event, musicInfo) => {
    const isWyOnlineList = isWyAccountListId(props.listId) || isWyDailyRecommendListId(props.listId)
    const wyPlaylistInfo = getWyAccountPlaylistInfo(props.listId)
    ...
    itemMenuControl.moveTo = !isWyOnlineList
    itemMenuControl.sort = !isWyOnlineList
    itemMenuControl.toggleSource = !isWyOnlineList
    itemMenuControl.remove = !isWyOnlineList || wyPlaylistInfo?.subscribed === false
