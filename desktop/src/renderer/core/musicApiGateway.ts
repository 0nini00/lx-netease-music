...
const fallbackQualityMap: Partial<Record<LX.Quality, string[]>> = {
  flac24bit: ['FLAC_24', 'FLAC', 'MP3_320', 'MP3_128'],
  flac: ['FLAC', 'MP3_320', 'MP3_128'],
  wav: ['WAV', 'FLAC', 'MP3_320', 'MP3_128'],
  ape: ['APE', 'FLAC', 'MP3_320', 'MP3_128'],
  '320k': ['MP3_320', 'MP3_128'],
  '192k': ['MP3_192', 'MP3_128'],
  '128k': ['MP3_128'],
}
...
const normalizeAuthValue = (keyHeader: string, key: string) => {
  if (keyHeader.toLowerCase() != 'authorization') return key
  return /^bearer\s+/i.test(key) ? key : `Bearer ${key}`
}

const getHeaders = () => {
  const key = appSetting['common.music_api_gateway_key'].trim()
  const keyHeader = appSetting['common.music_api_gateway_key_header'].trim() || 'X-API-Key'
  return key
    ? { [keyHeader]: normalizeAuthValue(keyHeader, key) }
    : {}
}
...
const getBaseUrl = () => {
  const baseUrl = appSetting['common.music_api_gateway_url'].trim()
...
const requestMusicUrlById = async({
  provider,
  id,
  quality,
}: {
...
export const isMusicApiGatewayEnabled = () => {
  return !!(appSetting['common.music_api_gateway_enable'] && appSetting['common.music_api_gateway_url'].trim())
}

export const getMusicApiGatewayUrl = async({
  musicInfo,
  quality,
}: {
  musicInfo: LX.Music.MusicInfoOnline
