...
const delayRetry = async(musicInfo: LX.Music.MusicInfo | LX.Download.ListItem, isRefresh = false, isAudioLoadFallback = false): Promise<string | null> => {
  // if (cancelDelayRetry) cancelDelayRetry()
  return new Promise<string | null>((resolve, reject) => {
    const time = getRandom(2, 6)
    setAllStatus(window.i18n.t('player__getting_url_delay_retry', { time }))
    const tiemout = setTimeout(() => {
      getMusicPlayUrl(musicInfo, isRefresh, true, isAudioLoadFallback).then((result) => {
...
const getMusicPlayUrl = async(musicInfo: LX.Music.MusicInfo | LX.Download.ListItem, isRefresh = false, isRetryed = false, isAudioLoadFallback = false): Promise<string | null> => {
  ...
  const skipPrimarySource = isRefresh && isAudioLoadFallback
  ...
  // eslint-disable-next-line @typescript-eslint/promise-function-async
    ...
    if (err.message == requestMsg.tooManyRequests) return delayRetry(musicInfo, isRefresh, isAudioLoadFallback)

    if (!isRetryed) return getMusicPlayUrl(musicInfo, isRefresh, true, isAudioLoadFallback)

    throw err
  })
}

export const setMusicUrl = (musicInfo: LX.Music.MusicInfo | LX.Download.ListItem, isRefresh?: boolean, isAudioLoadFallback = false) => {
  // if (appSetting['player.autoSkipOnError']) addLoadTimeout()
  if (!diffCurrentMusicInfo(musicInfo)) return
  if (cancelDelayRetry) cancelDelayRetry()
  gettingUrlId = createGettingUrlId(musicInfo)
  void getMusicPlayUrl(musicInfo, isRefresh, false, isAudioLoadFallback).then((url) => {
    if (!url) return
    setResource(url)
  }).catch((err: any) => {
    console.log(err)
