...
import { getMusicApiGatewayUrl, isMusicApiGatewayEnabled } from '@renderer/core/musicApiGateway'
...
interface RuntimeUserApiSource {
  getMusicUrl?: (songInfo: Record<string, any>, type: LX.Quality) => {
    promise: Promise<{ url: string, type: LX.Quality }>
    canceleFn?: () => void
  }
}

const getRuntimeUserApiSource = (source: LX.Source): RuntimeUserApiSource | null => {
  return (userApi.apis as Partial<Record<LX.Source, RuntimeUserApiSource>>)[source] ?? null
}
...
const buildSourceFallbackQualitys = ({
  musicInfo,
  quality,
  sourceQualitys,
}: {
  ...
  quality: LX.Quality
  sourceQualitys: LX.Quality[]
}) => {
  const qualitys: LX.Quality[] = []
  const appendQuality = (targetQuality: LX.Quality) => {
    if (!sourceQualitys.includes(targetQuality)) return
    if (!musicInfo.meta._qualitys[targetQuality]) return
    if (!qualitys.includes(targetQuality)) qualitys.push(targetQuality)
  }

  appendQuality(quality)
  appendQuality(getPlayQuality(appSetting['player.playQuality'], musicInfo))
  for (const sourceQuality of sourceQualitys) appendQuality(sourceQuality)

  return qualitys
}

const buildUserApiFallbackQualitys = buildSourceFallbackQualitys
...
export const getOnlineOtherSourceMusicUrl = async({ musicInfos, quality, onToggleSource, isRefresh, retryedSource = [] }: {
  ...
  while (musicInfo = (musicInfos.shift()!)) {
    ...
    itemQualitys = buildSourceFallbackQualitys({
      musicInfo,
      quality: quality ?? getPlayQuality(appSetting['player.playQuality'], musicInfo),
      sourceQualitys,
    })
  ...
  for (const itemQuality of itemQualitys) {
...
let fallbackUserApiQueue = Promise.resolve()

const requestCurrentUserApiMusicUrl = async({
  musicInfo,
  quality,
}: {
  musicInfo: LX.Music.MusicInfoOnline
  ...
  const api = getRuntimeUserApiSource(musicInfo.source)
  if (!api?.getMusicUrl) throw new Error('user api source is not support')
  return api.getMusicUrl(toOldMusicInfo(musicInfo), quality).promise.then(({ url, type }: { url: string, type: LX.Quality }) => ({
    url,
    quality: type,
  }))
}

const runWithFallbackUserApiLock = async<T>(handler: () => Promise<T>) => {
  const prevQueue = fallbackUserApiQueue
  let release: () => void
  fallbackUserApiQueue = new Promise(resolve => {
    release = resolve
  })
  await prevQueue
  try {
...
const getUserApiFallbackMusicUrl = async({
  musicInfo,
  quality,
}: {
  musicInfo: LX.Music.MusicInfoOnline
  ...
  if (!/^user_api/.test(apiSource.value || '')) throw new Error('not user api source')

  const originalApiId = apiSource.value
  const currentApiId = appSetting['common.apiSource']
  const candidateApis = userApi.list.filter(api => (
    /^user_api/.test(api.id) &&
    api.id != currentApiId
  ))

  if (!candidateApis.length) throw new Error(window.i18n.t('toggle_source_failed'))

  return runWithFallbackUserApiLock(async() => {
    try {
      for (const api of candidateApis) {
        try {
          await setUserApi(api.id)
          if (!await window.lx.apiInitPromise[0]) continue

          const sourceInfo = getRuntimeUserApiSource(musicInfo.source)
          const sourceQualitys = sourceInfo ? qualityList.value[musicInfo.source] : null
          if (!sourceInfo?.getMusicUrl || !sourceQualitys?.length) continue

          for (const targetQuality of buildUserApiFallbackQualitys({ musicInfo, quality, sourceQualitys })) {
            try {
              console.log('fallback user api try:', api.name, musicInfo.source, targetQuality, musicInfo.name)
              const result = await requestCurrentUserApiMusicUrl({ musicInfo, quality: targetQuality })
              console.log('fallback user api success:', api.name, musicInfo.source, result.quality, musicInfo.name)
              return result
            } catch (err) {
              console.log('fallback user api failed:', api.name, musicInfo.source, targetQuality, err)
            }
          }
        } catch (err) {
          console.log('fallback user api failed:', api.name, err)
        }
      }
      throw new Error(window.i18n.t('toggle_source_failed'))
    } finally {
      if (originalApiId && apiSource.value != originalApiId) {
        await setUserApi(originalApiId)
      }
    }
  })
}
 ...
 * 获取在线音乐URL
 */
export const handleGetOnlineMusicUrl = async({ musicInfo, quality, onToggleSource, isRefresh, allowToggleSource, skipPrimarySource = false }: {
  musicInfo: LX.Music.MusicInfoOnline
  quality?: LX.Quality
  isRefresh: boolean
  ...
  let reqPromise
  if (skipPrimarySource) {
    reqPromise = Promise.reject(new Error('skip primary source'))
  } else {
    try {
  ...
  return reqPromise.then(({ url, type }: { url: string, type: LX.Quality }) => {
    ...
    if (/^user_api/.test(apiSource.value || '')) {
      try {
        const result = await getUserApiFallbackMusicUrl({
          musicInfo,
          quality: targetQuality,
        })
        return {
