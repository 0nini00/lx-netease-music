...
app.on('ready', () => {
  global.lx.event_app.on('main_window_created', (win) => {
    ...
    void installExtension(VUEJS_DEVTOOLS, { session: win.webContents.session })
      .then((name: string) => {
        console.log(`[main window] Added Extension:  ${name}`)
      })
      .catch((err: Error) => {
        console.log('[main window] Skip devtools extension:', err.message)
      })
  })
  global.lx.event_app.on('desktop_lyric_window_created', (win) => {
    ...
    void installExtension(VUEJS_DEVTOOLS, { session: win.webContents.session })
      ...
      .catch((err: Error) => {
        console.log('[lyric window] Skip devtools extension:', err.message)
      })
  })
})
