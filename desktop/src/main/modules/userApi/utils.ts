...
export const getUserApisFull = (): LX.UserApi.UserApiInfoFull[] => {
  return getUserApis().map(api => ({
    ...api,
    script: scripts.get(api.id) ?? '',
  }))
}

export const overwriteUserApisFull = (apis: LX.UserApi.UserApiInfoFull[]) => {
  userApis = []
  scripts = new Map()
  for (const api of apis) {
    if (!api.id || !api.script) continue
    const { script, ...info } = api
    userApis.push({
      ...info,
